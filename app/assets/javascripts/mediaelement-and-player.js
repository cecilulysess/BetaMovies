...
mejs.MediaFeatures = {
    ...
    // HACK: Disable native / semi-native full screen
    t.hasSemiNativeFullScreen = false;
    t.hasTrueNativeFullScreen = false;
    t.hasWebkitNativeFullScreen = false;
    t.hasMozNativeFullScreen = false;
		...
		keyActions: [
				...
				},
				{
						...
						action: function(player, media) {
								if (!isNaN(media.duration) && media.duration > 0) {
										// 5%
										var newTime = Math.max(media.currentTime - 10, 0);
				...
				},
				{
						...
						action: function(player, media) {
								if (!isNaN(media.duration) && media.duration > 0) {
										// 5%
										var newTime = Math.min(media.currentTime + 10, media.duration);
										media.setCurrentTime(newTime);
								}
						}
				},
	...
	mejs.MediaElementPlayer.prototype = {
			...
			if ((mf.isiPad && t.options.iPadUseNativeControls) || (mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				*/
				...
				if (t.options[tagName + 'Height'] > 0 || t.options[tagName + 'Height'].toString().indexOf('%') > -1) {
					t.height = t.options[tagName + 'Height'] + t.options.controlsHeight; // Hack to leave space for controls
			...
			if (!(mf.isAndroid && t.options.AndroidUseNativeControls) && !(mf.isiPad && t.options.iPadUseNativeControls) && !(mf.isiPhone && t.options.iPhoneUseNativeControls)) {				
				...
				if (t.isVideo) {
					...
					if (mejs.MediaFeatures.hasTouch) {
						...
						// Removed pause on click because 
						// 1) We removed the play button overlay and 
						// 2) It's not compatible with slide viewer
				...
				$(window).resize(function() {
                    ...
                    if(t.isFullScreen){
                    	// HACK: Run only if t.resizeFullScreen switch is undefined or set to true
                    	if(typeof(t.resizeFullScreen) !== 'boolean' || t.resizeFullScreen){
                        	t.media.setVideoSize($(window).width(),$(window).height() - t.options.controlsHeight);
                    	}
                    }
				});				
		...
		setPlayerSize: function(width,height) {
			...
			if (t.height.toString().indexOf('%') > 0) {
				...
				t.layers.children('.mejs-layer')
					.width(t.width)
					.height(t.height - t.options.controlsHeight);
		...
		buildkeyboard: function(player, controls, layers, media) {
				...
				$(document).keydown(function(e) {
                        ...
                        // Does not require player focus // player.hasFocus && 
	...
	$.extend(MediaElementPlayer.prototype, {
		...
		exitFullScreen: function() {
			...
			if (t.pluginType === 'native') {
				t.$media
					.width(normalWidth)
					.height(normalHeight - t.options.controlsHeight);
			} else {
				t.container.find('object embed')
					.width(normalWidth)
					.height(normalHeight - t.options.controlsHeight);
					
				t.media.setVideoSize(normalWidth, normalHeight - t.options.controlsHeight);
			}				
